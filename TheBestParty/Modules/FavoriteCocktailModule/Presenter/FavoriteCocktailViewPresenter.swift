...
protocol FavoriteCocktailViewPresenterProtocol: AnyObject {
    ...
    var realmCocktails: [CocktailRealmModel]? { get set }
...
final class FavoriteCocktailViewPresenter: FavoriteCocktailViewPresenterProtocol {
    ...
    var realmCocktails: [CocktailRealmModel]?
    ...
    public func configureCellForItemAt(index: Int) -> CocktailModel? {
        let realm = try! Realm()
        let dogs = realm.objects(CocktailRealmModel.self)
        let result = dogs[index].cocktailsRealm
        
        return result
//        cell.configureCell(with: dogs[indexPath.row].cocktailsRealm!)
    }
}
